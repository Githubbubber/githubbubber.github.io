(function ($) {
	...
	$("#menu-handler").on("click", function () {
		...
		TweenMax.to($("#curve_open"), 0.4, {
			attr: { d: $("#curve_open").data("end-d") },
		});
		setTimeout(function () {
			$(".header-close").show();
			$(".header-tool").hide();
			TweenMax.to($("#curve_close"), 0.4, {
				attr: { d: $("#curve_close").data("end-d") },
			});
		}, 400);
	});
	...
	$("#menu-close").on("click", function () {
		...
		TweenMax.to($("#curve_close"), 0.4, {
			attr: { d: $("#curve_close").data("start-d") },
		});
		setTimeout(function () {
			$(".header-close").hide();
			$(".header-tool").show();
			TweenMax.to($("#curve_open"), 0.4, {
				attr: { d: $("#curve_open").data("start-d") },
			});
		}, 400);
	});
				...
				function (direction) {
					$chart.find("svg").each(function (svg_index) {
						...
						$svg.find("path").each(function () {
							...
							_path = _path.replace(
								/XX/gi,
								500 - (500 * _percent) / 100
							);
							TweenMax.to($path, 0.5 + svg_index * 0.15, {
								attr: { d: _path },
							});
						});
					});
